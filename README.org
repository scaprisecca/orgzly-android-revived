...
For local F-Droid builds on this repo, the preferred helpers are:
...
- create ~local.properties~ if it is missing
- default to a useful local build task
...
./scripts/build_fdroid.sh assembleFdroidRelease
./scripts/build_fdroid.sh bundleFdroidRelease
#+end_src

Side-by-side dev build commands:
...
The dev build uses:
- application ID: ~com.orgzlyrevived.dev~
- app label: ~Orgzly Revived Dev~

That lets it coexist with the installed store/F-Droid app for manual testing.

Environment overrides:

#+begin_src shell
JAVA17_HOME=/path/to/jdk17 ANDROID_SDK_ROOT=/path/to/android-sdk ./scripts/build_fdroid.sh
JAVA17_HOME=/path/to/jdk17 ANDROID_SDK_ROOT=/path/to/android-sdk ./scripts/build_fdroid_dev.sh
#+end_src

APK outputs:
- standard debug APK: ~app/build/outputs/apk/fdroid/debug/app-fdroid-debug.apk~
- dev APK: ~app/build/outputs/apk/fdroid/dev/app-fdroid-dev.apk~
- release APK: ~app/build/outputs/apk/fdroid/release/app-fdroid-release-unsigned.apk~

Note: the release APK is unsigned unless you add your own signing config, so the standard debug or dev APK is the quickest option for manual testing on a phone.
